...
case when main_entity='Cash Voucher' and type='credit' then 'Cash Receipt'
    when main_entity='Cash Voucher' and type='debit'  then 'Cash Payment'
            When main_entity <> 'Cash Voucher' and type ='credit' then 'Adjustment Receipt'
            when  main_entity <> 'Cash Voucher' and type ='debit' then 'Adjustment Payment' end as Type  from account_entry a join ledger l on l.code=a.ledger_code where   main_entity not like '%Cash Voucher%'  and ledger_code not in ('F007','Z001','G001','G002','G003','G004','C001','E003','L001','L002','L003','U001','U002','U003','U004','U005','U006','D001','F001','A003','F002','H001','P003','O022','O017','P004','D002','P001')
...
union
SELECT '','','',date,'G001','SURETY LOAN',(CASE WHEN (main_entity='DEMAND COLLECTION') && (main_entity_type='EXCESS COLLECTION' || main_entity_type='FULL COLLECTION' || main_entity_type='LESS COLLECTION')THEN 'AS PER COL' ELSE '' END) as narration,sum(amount) as amount,
case when main_entity='Cash Voucher' and type='credit' then 'Cash Receipt' when main_entity='Cash Voucher' and type='debit'  then 'Cash Payment' When main_entity <> 'Cash Voucher' and type ='credit' then 'Adjustment Receipt' when  main_entity <> 'Cash Voucher' and type ='debit' then 'Adjustment Payment' end as Type FROM account_entry WHERE (ledger_code='G001') AND main_entity_type in ('EXCESS COLLECTION','FULL COLLECTION','LESS COLLECTION') and (main_entity<>'loan') group by type,'SURETY LOAN',date
union
SELECT '',reference,'',date,'G001','SURETY LOAN',(CASE WHEN (main_entity='voucher') && (main_entity_type='ADJUSTMENT RECEIPT')THEN 'AS PER COL' ELSE '' END) as narration,sum(amount) as amount,case when main_entity='Cash Voucher' and type='credit' then 'Cash Receipt' when main_entity='Cash Voucher' and type='debit'  then 'Cash Payment' When main_entity <> 'Cash Voucher' and type ='credit' then 'Adjustment Receipt'    when  main_entity <> 'Cash Voucher' and type ='debit' then 'Adjustment Payment' end as Type FROM account_entry WHERE (ledger_code='G001') AND main_entity_type in ('ADJUSTMENT RECEIPT') and (main_entity<>'loan') group by reference,type,'SURETY LOAN',date
union
...
union
SELECT '',reference,'',date,'G002',' FESTIVAL LOAN',(CASE WHEN main_entity='loan' THEN 'AS PER LDR'
WHEN (main_entity='DEMAND COLLECTION' || main_entity='voucher') && (main_entity_type='ADJUSTMENT RECEIPT' || main_entity_type='EXCESS COLLECTION' || main_entity_type='FULL COLLECTION' || main_entity_type='LESS COLLECTION')THEN 'AS PER COL'
ELSE '' END) as narration,
sum(amount) as amount,
case when main_entity='Cash Voucher' and type='credit' then 'Cash Receipt'
    when main_entity='Cash Voucher' and type='debit'  then 'Cash Payment'
            When main_entity <> 'Cash Voucher' and type ='credit' then 'Adjustment Receipt'
            when  main_entity <> 'Cash Voucher' and type ='debit' then 'Adjustment Payment' end as Type
FROM account_entry WHERE (ledger_code='G002' ) and (main_entity<>'loan')  and  main_entity_type<>'ADJUSTMENT SLIP' group by reference,type,' FESTIVAL LOAN',date
union
...
union
SELECT '' as id,reference,'' as search_no,date,'G004' as ledger_code,'FLOOD LOAN',(CASE WHEN main_entity='loan'  THEN 'AS PER LDR'
WHEN (main_entity='DEMAND COLLECTION' || main_entity='voucher') && (main_entity_type='ADJUSTMENT RECEIPT' || main_entity_type='EXCESS COLLECTION' || main_entity_type='FULL COLLECTION' || main_entity_type='LESS COLLECTION')THEN 'AS PER COL'
ELSE '' END) as narration,
sum(amount) as amount,
case when main_entity='Cash Voucher' and type='credit' then 'Cash Receipt'
...
union
SELECT '',reference,'',date,'E003',' SRF',(CASE WHEN main_entity='loan' || main_entity='staff loan' THEN 'AS PER LDR'
WHEN (main_entity='DEMAND COLLECTION' || main_entity='voucher') && (main_entity_type='ADJUSTMENT RECEIPT' || main_entity_type='EXCESS COLLECTION' || main_entity_type='FULL COLLECTION' || main_entity_type='LESS COLLECTION')THEN 'AS PER COL'
ELSE '' END) as narration,
sum(amount) as amount,
case when main_entity='Cash Voucher' and type='credit' then 'Cash Receipt'
...
union
SELECT '','','',date,ledger_code,l.name,
(CASE WHEN main_entity='loan' || main_entity='staff loan' THEN 'AS PER LDR' 
WHEN (main_entity='DEMAND COLLECTION') && (main_entity_type='EXCESS COLLECTION' || main_entity_type='FULL COLLECTION' || main_entity_type='LESS COLLECTION')THEN 'AS PER COL' ELSE '' END) as narration,sum(amount) as amount,
case when main_entity='Cash Voucher' and type='credit' then 'Cash Receipt' when main_entity='Cash Voucher' and type='debit' then 'Cash Payment' When main_entity <> 'Cash Voucher' and type ='credit' then 'Adjustment Receipt' when  main_entity <> 'Cash Voucher' and type ='debit' then 'Adjustment Payment' end as Type1 FROM account_entry a join ledger l on l.code=a.ledger_code WHERE ledger_code not in
('Z001','G001','G002','G003','G004','U001','U002','U003','U004','A001','P003','O001','E003','C001','Z005','B001','B003','I002','I001','M004','M005','M006','F004','F003','F010','F011','F012','F013','F015','G006','G007','G008','G009','K004','L003','L004') and main_entity_type not in 
('ADJUSTMENT RECEIPT','ADJUSTMENT SLIP','Renewal FD') and main_entity <>'Cash Voucher' group by type1,date,ledger_code,l.name
union
SELECT '',reference,'',date,ledger_code,l.name,
(CASE WHEN main_entity='loan' || main_entity='staff loan' THEN 'AS PER LDR' 
WHEN (main_entity='voucher') && (main_entity_type='ADJUSTMENT RECEIPT')THEN 'AS PER COL' ELSE '' END) as narration,sum(amount) as amount,
case when main_entity='Cash Voucher' and type='credit' then 'Cash Receipt' when main_entity='Cash Voucher' and type='debit' then 'Cash Payment' When main_entity <> 'Cash Voucher' and type ='credit' then 'Adjustment Receipt' when  main_entity <> 'Cash Voucher' and type ='debit' then 'Adjustment Payment' end as Type1 FROM account_entry a join ledger l on l.code=a.ledger_code WHERE ledger_code not in ('Z001','G001','G002','G003','G004','U001','U002','U003','U004','A001','P003','O001','E003','C001','Z005','B001','B003','I002','I001','M004','M005','M006','F004','F003','F010','F011','F012','F013','F015','G006','G007','G008','G009','K004','L003','L004') and main_entity_type='ADJUSTMENT RECEIPT' and main_entity <>'Cash Voucher' group by reference,type1,date,ledger_code,l.name
union
SELECT a.id,reference,search_no,date,ledger_code,l.name,narration,amount,
case when main_entity='Cash Voucher' and type='credit' then 'Cash Receipt'
when main_entity='Cash Voucher' and type='debit'  then 'Cash Payment'
