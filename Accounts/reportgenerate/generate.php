...
case when main_entity='Cash Voucher' and type='credit' then 'Cash Receipt'
    when main_entity='Cash Voucher' and type='debit'  then 'Cash Payment'
            ...
            union
            SELECT a.id,reference,v.name as search_no,a.date,a.ledger_code,l.name,'Fd Interest Payable',sum(a.amount),
case when main_entity='Cash Voucher' and type='credit' then 'Cash Receipt'
    when main_entity='Cash Voucher' and type='debit'  then 'Cash Payment'
            When main_entity <> 'Cash Voucher' and type ='credit' then 'Adjustment Receipt'
...
case when main_entity='Cash Voucher' and type='credit' then 'Cash Receipt'
    when main_entity='Cash Voucher' and type='debit'  then 'Cash Payment'
            When main_entity <> 'Cash Voucher' and type ='credit' then 'Adjustment Receipt'
            when  main_entity <> 'Cash Voucher' and type ='debit' then 'Adjustment Payment' end as Type  from account_entry a join ledger l on l.code=a.ledger_code where   main_entity not like '%Cash Voucher%'  and ledger_code not in ('F007','Z001','G001','G002','G003','G004','G007','M005','M004','C001','E003','L002','L001','L003','U001','U002','U003','U004','U005','U006','D001','F001','A003','I002','I001','F002','H001','P003','F004','F011','F015','F003','G006','O022','O017','P004','D002','P001','G008','L004','B001','B003','J003')
...
case when main_entity='Cash Voucher' and type='credit' then 'Cash Receipt'
    when main_entity='Cash Voucher' and type='debit'  then 'Cash Payment'
            When main_entity <> 'Cash Voucher' and type ='credit' then 'Adjustment Receipt'
            when  main_entity <> 'Cash Voucher' and type ='debit' then 'Adjustment Payment' end as Type
FROM account_entry WHERE (ledger_code='M005' ) and (main_entity<>'loan' || main_entity<>'staff loan') AND main_entity_type<>'ADJUSTMENT SLIP' group by type,'STAFF LOAN',date
...
FROM account_entry a join ledger l on l.code=a.ledger_code WHERE ledger_code not in ('Z001','G001','G002','G003','G004','M005','U001','U002','U003','U004','A001','P003','O001','E003','C001','Z005','M006','F012','K004','F010','G009','G010') and   main_entity_type not in ('ADJUSTMENT SLIP','Renewal FD')  and main_entity <>'Cash Voucher' group by type1,date,ledger_code,l.name
union
SELECT a.id,reference,search_no,date,ledger_code,l.name,narration,amount,
case when main_entity='Cash Voucher' and type='credit' then 'Cash Receipt'
when main_entity='Cash Voucher' and type='debit'  then 'Cash Payment'
...
and ((narration <> 'Fd Interest Payable' and narration <>'Fixed Deposit Interest Payable' and narration<>'FD RENEWAL') or narration is null)
