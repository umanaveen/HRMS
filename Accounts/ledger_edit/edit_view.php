 ...
 if (is_numeric($code)) 
	{
       ?>
		...
		<thead>
		<tr>
		<th COLSPAN='3' align="center">MEMBER LEDGER &nbsp;&nbsp; BALANCE &nbsp;&nbsp;ON&nbsp;&nbsp;<?php echo date('d-m-Y',strtotime($date));?></th>
		</tr>
		</thead>
		<?php 
...
<?php			
	...
	<?php 
	$query="SELECT id,date,ledger_code,balance FROM ledger_opening_balance WHERE date='$from_date' and ledger_code='$ledger_code'";
	$result=mysql_fetch_array(mysql_query($query));
	?>
	<tbody>
	<tr><td>DATE</td>
...
function ledger_update()
{
	...
	data:'&ledger='+ledger+'&date='+date+'&balance='+balance,
