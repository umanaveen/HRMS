...
<div class="row">
    ...
    <thead>
      ...
      <tbody>
      <?php
      $emp_sql=$con->query("SELECT *,j.status as status,j.id as jid FROM `jobdescription_form_details` j join jobdescription_master m on j.jobdescription_id=m.id left join client_master c on j.client_id=c.id  order by j.id desc");
      $i=1;
      ...
      <tr>
		  ...
		  <button class="btn btn-success btn-sm" data-id="<?php echo $emp_res['jid']; ?>" onclick="jd_allocate(<?php echo $emp_res['jid']; ?>)"> Allocate</button>
...
function back()
 ...
 <script>
	...
	success:function(data)
	{
 	...
 	$.ajax({
	type:"POST",
	url:"/HRMS/HRMS/resource/jobdescription_form/jd_view.php?jid="+v,
	success:function(data)
	{
		$("#main_content").html(data);
	}
	...
	function jd_close(v)
	  {		 
		  $.ajax({
	type:"POST",
	url:"/HRMS/HRMS/resource/jobdescription_form/jd_close.php?jid="+v,
	success:function(data)
	{
		$("#main_content").html(data);
	}
