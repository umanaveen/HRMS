...
$sql=$con->query("SELECT * FROM resource_form_detail s left join jobdescription_master d on s.position=d.id join source_master sm on s.source=sm.id left join interview_schedule_detail i on s.id=i.resource_id where s.id='$resourceid'");
$fet=$sql->fetch();
?>
...
<tr>
		...
		?>
			<tr>
		<td colspan="6"><center><b>Schedule Details</center></b></td>
		</tr>
		<tr>
		 <td>Feedback:</td>
        <td colspan="4">	<?php if($fet['feedback']==1){?>
		<input type="text" class="form-control" id="feedback" name="feedback" value="Not Selected"readonly>
		<?php }	elseif($fet['feedback']==2){?>
		<input type="text" class="form-control" id="feedback" name="feedback" value="Schedule Interview"readonly>
		<?php }
		?>
		</td>
        </tr>
        ...
        <td colspan="4">	
		<input type="text" class="form-control" id="int_date" name="int_date" value="<?php echo $fet['interview_date']; ?>"readonly>
		</td>
        </tr>
		 <tr>
		 <td>Remarks:</td>
        <td colspan="4">	
		<input type="text" class="form-control" id="remarks" name="remarks" value="<?php echo $fet['remarks']; ?>"readonly>
		</td>
        </tr>
