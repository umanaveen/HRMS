...
<table id="example1" class="table table-bordered">
      ...
      <tbody>
      <?php
      $emp_sql=$con->query("SELECT *,c.id as cid,c.status as status FROM `candidate_form_details` c left join company_master cm on c.company_name=cm.id join designation_master d on c.position=d.id join z_department_master z on c.department=z.id where c.status=2 or c.status=3 or c.status=4 or c.status=6 or c.status=20 or c.status=5 or c.status=7 or c.status=8 or c.status=9 or c.status=13 or c.status=14 or c.status=15 or c.status=16 or c.status=17 or c.status=18 or c.status=19 or c.status=20 or c.status=21 or c.status=22 or c.status=23 order by c.id desc");
      $i=1;
...
?>
		...
		<span style="color:orange;text-align:center;"><b>Active</b></span>
		  ...
		  else if($emp_res['status'] == 4 || $emp_res['status'] == 6 || $emp_res['status'] == 3 || $emp_res['status'] == 20|| $emp_res['status'] == 14 || $emp_res['status'] == 15||  $emp_res['status'] == 17|| $emp_res['status'] == 18|| $emp_res['status'] == 19|| $emp_res['status'] == 20|| $emp_res['status'] == 21 || $emp_res['status'] == 23)
		  {
			 ?> 
