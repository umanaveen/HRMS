...
require '../../connect.php';
$cid=$_REQUEST['id'];
$sql=$con->query("select * from candidate_form_details c left join company_master cm on c.company_name=cm.id join designation_master d on c.position=d.id left join z_department_master dm on c.department=dm.id where c.id='$cid' order by c.id desc limit 1");
$fet=$sql->fetch();
?>
...
function staff_code_updation()
{
	...
	$.ajax({
		...
		url:'/HRMS/HRMS/candidate/update_staff_code.php',
