...
{
	?>
	<option value="<?php echo $dis['id'];?>"><?php echo $dis['designation_name'];?></option>
	<?php
}
?>
</td>
...
function back_ctc()
{
	...
	function joining_update(v)
	{
		var desig=$('#approved_desig').val();
		var ctc=$('#approved_ctc').val();
		var jdate=$('#joining_date').val();
		
	$.ajax({
	type:"POST",
	url:"HRMS/candidate/send_application_form.php?id="+v+"&desig="+desig+"&ctc="+ctc+"&jdate="+jdate,
	success:function(data)
	{
