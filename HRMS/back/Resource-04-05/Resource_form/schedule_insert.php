...
$fet=$sel->fetch();
echo $user_name=$fet['first_name']; 

 if($feedback==2)
{
	echo "hi";
	$sql=$con->query("insert into interview_schedule_detail (resource_id,feedback,interview_date,remarks,status,user_role,user_name,password,created_by,created_on)values('$resource_id','$feedback','$interview_date','$remarks','$status','$role','$user_name','$password','$userid',now())");
