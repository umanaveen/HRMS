...
$sql=$con->query("SELECT * FROM resource_form_detail s left join designation_master d on s.position=d.id join source_master sm on s.source=sm.id join interview_schedule_detail i on s.id=i.resource_id where s.id='$resourceid'");
$fet=$sql->fetch();
?>
...
?>				
		...
		<tr>
		 <td>Feedback:</td>
        <td colspan="4">	
		<input type="text" class="form-control" id="feedback" name="feedback" value="<?php echo $fet['feedback']; ?>"readonly>
		</td>
        </tr>
		 <tr>
