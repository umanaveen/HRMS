...
$allowTypes1 = array('pdf', 'doc', 'docx', 'jpg', 'png', 'jpeg'); 
$response = array( 
    //'status' => 0, 
...
/* photo upload */
			$fileNames1 = array_filter($photo['name']); 
...
if($EmployeeStatus=="new")
{
	...
	$inserts=$con->query("INSERT INTO candidate_form_details(resource_id,company_name,position, department, first_name, last_name, father_name, gender, dob, address, paddress, phone,alternative_phone, mail, adharnumber, pannumber, voternumber, educationalDetails, EmployeeStatus, year_of_pass, resume,photo,status, created_by, created_on) VALUES ('$resource_id','$company','$position', '$tech_department', '$first_name', '$last_name', '$father_name', '$gender', '$dob', '$address', '$paddress', '$phone', '$a_phone','$mail', '$adharnumber', '$pannumber', '$voternumber', '$educationalDetails', '$EmployeeStatus', '$year_of_pass','$fileNames','$fileNames1', '$status','$userid','$date')"); 
...
$res = $edit_id->fetch();
 $candidate_id=$res['id']; 
	if($inserts)
	{
		...
		$insert=$con->query("insert into z_user_master(department,candidate_id,user_name,password,full_name,status,email_id,user_group_code,mobile_no,gender,created_by,created_on)values('$tech_department','$candidate_id','$phone','$password','$full_name','1','$mail','ROLE-011','$phone','$gender','$userid','$date')");
...
$inserts=$con->query("INSERT INTO candidate_form_details(resource_id,company_name,position, department, first_name, last_name, father_name, gender, dob, address, paddress, phone, alternative_phone,mail, adharnumber, pannumber, voternumber, educationalDetails, EmployeeStatus,  companyname, no_of_year,resume,photo, status, created_by, created_on) VALUES ('$resource_id','$company','$position',  '$tech_department', '$first_name', '$last_name', '$father_name','$gender',  '$dob', '$address', '$paddress', '$phone','$a_phone', '$mail', '$adharnumber', '$pannumber', '$voternumber','$educationalDetails', '$EmployeeStatus',  '$companyname', '$no_of_year','$fileNames','$fileNames1','$status','$userid','$date')");
