...
<form method="POST" action="HRMS/Recruitment/staff_asset/asset_recollect_submit.php">
...
<?php
$isel=$con->query("select distinct m.id as id,name,a.Serial_no as Serial_no from assets_form_detail a join assets_master m on a.asset_name=m.id where a.asset='Internal Asset' and m.id in($access) and a.id in(select asset_id from staff_asset_list where staff_id='$sid' and status=1) ");
...
<td>
<?php
$isel=$con->query("select distinct m.id as id,name,a.Serial_no as Serial_no,a.id as aid from assets_form_detail a join assets_master m on a.asset_name=m.id where a.asset='Internal Asset' and m.id in($access) and a.id in(select asset_id from staff_asset_list where staff_id='$sid' and status=2)");
$i=1;
$s=1;
while($dfet=$isel->fetch())
{
...
<script>
    ...
    $.ajax({
    type:"POST",
    url:"/HRMS/HRMS/Recruitment/staff_asset/staff_assets_recollect.php",
