...
require '../../../connect.php';
 $id=$_REQUEST['id'];
$stmt = $con->prepare("SELECT name,s.asset_master_id as asset_master_id,s.cug_status as cug_status,s.*,a.*,f.*,m.*,sm.* FROM `staff_access_request` s join staff_asset_list a on s.id=a.asset_request_id join assets_form_detail f on a.asset_id=f.id join assets_master m on f.asset_name=m.id join sim_master sm on a.sim_id=sm.id where s.id='$id'");
$stmt->execute(); 
$row = $stmt->fetch();
