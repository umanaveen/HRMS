   ...
   <div class="panel-body">
    ...
    <thead>
     ...
     <th>Action</th>
	  ...
	  if($staff_id=='')
	  {
		 $emp_sql=$con->query("select *,r.id as sid from staff_asset_list a join staff_master m on a.staff_id=m.id join staff_access_request r on a.asset_request_id=r.id where a.status=1 group by a.staff_id"); 
	  }
	 ...
	 //echo "SELECT sm.emp_name,s.stationaries,s.system_or_laptop,s.id_card,s.cug,s.access_card,s.erp_access,s.mail_id,s.id AS sid FROM staff_asset s join staff_master sm on s.emp_name=sm.id";
      ...
      {
		 echo "hii". $staffid=$emp_res['staff_id'];
       ?>
      <tr>
      <td><?php echo $i; ?></td>
...
<script>
  ...
  function staff_asset_return_hr(v)
    ...
    $.ajax({
    type:"POST",
    url:"/HRMS/HRMS/Recruitment/staff_asset/get_assets_return.php?id="+v,
    success:function(data){
    $("#main_content").html(data);
    }
    })
