...
$id=$_REQUEST['id'];
$stmt = $con->prepare("SELECT * FROM candidate_form_details c left join  z_department_master d on c.department=d.id left join designation_master dm on c.position=dm.id
WHERE c.id='$id'"); 
$stmt->execute(); 
$row = $stmt->fetch();
/* echo "SELECT * FROM candidate_form_details c left join  z_department_master d on c.department=d.id left join designation_master dm on c.position=dm.id
WHERE c.id='$id'"; */
?>
...
<form method="POST" action="Qvision/interview_feedback/hr_submit.php">
...
function back()
 ...
 $("#main_content").html(data);
  ...
  data:"id="+id+"&user_id="+user_id+"&interviewroundid="+interviewroundid+"&intername_id="+intername_id, data,
    url:'HRMS/interview_feedback/hr_submit.php',
